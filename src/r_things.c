...
static void R_SplitSprite(vissprite_t *sprite)
{
	...
	for (i = 1; i < sector->numlights; i++)
	{
		...
		if (!(sector->lightlist[i].caster->flags & FF_NOSHADE))
		{
			...
			{
				lindex = FixedMul(sprite->xscale, LIGHTRESOLUTIONFIXED)>>(LIGHTSCALESHIFT);
...
//
	...
	{
		// diminished light
		lindex = FixedMul(xscale, LIGHTRESOLUTIONFIXED)>>(LIGHTSCALESHIFT);
	...
	for (rover = vsprsortedhead.prev; rover != &vsprsortedhead; rover = rover->prev)
	{
		...
		for (r2 = head->next; r2 != head; r2 = r2->next)
		{
			...
			else if (r2->thickseg)
			{
				...
				scale = FLOAT_TO_FIXED(r2->thickseg->scale1) > FLOAT_TO_FIXED(r2->thickseg->scale2) ? FLOAT_TO_FIXED(r2->thickseg->scale1) : FLOAT_TO_FIXED(r2->thickseg->scale2);
			...
			else if (r2->seg)
			{
				...
				scale = FLOAT_TO_FIXED(r2->seg->scale1) > FLOAT_TO_FIXED(r2->seg->scale2) ? FLOAT_TO_FIXED(r2->seg->scale1) : FLOAT_TO_FIXED(r2->seg->scale2);
				if (scale <= rover->sortscale)
					continue;
	...
	for (; clippedvissprites < visspritecount; clippedvissprites++)
	{
		...
		for (ds = ds_p; ds-- > dsstart;)
		{
			...
			if (ds->portalpass != 66)
			{
				...
				if (FLOAT_TO_FIXED(scale) < spr->sortscale ||
					(FLOAT_TO_FIXED(lowscale) < spr->sortscale &&
