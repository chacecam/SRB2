...
// ttmode old
static patchpointer_t ttbanner; // white banner with "robo blast" and "2"
static patchpointer_t ttwing; // wing background
static patchpointer_t ttsonic; // "SONIC"
static patchpointer_t ttswave1; // Title Sonics
static patchpointer_t ttswave2;
static patchpointer_t ttswip1;
static patchpointer_t ttsprep1;
static patchpointer_t ttsprep2;
static patchpointer_t ttspop1;
static patchpointer_t ttspop2;
static patchpointer_t ttspop3;
static patchpointer_t ttspop4;
static patchpointer_t ttspop5;
static patchpointer_t ttspop6;
static patchpointer_t ttspop7;
...
static patchpointer_t ttribb[6][TTMAX_ALACROIX];
static patchpointer_t ttsont[6][TTMAX_ALACROIX];
static patchpointer_t ttrobo[6][TTMAX_ALACROIX];
static patchpointer_t tttwot[6][TTMAX_ALACROIX];
static patchpointer_t ttembl[6][TTMAX_ALACROIX];
static patchpointer_t ttrbtx[6][TTMAX_ALACROIX];
static patchpointer_t ttsoib[6][TTMAX_ALACROIX];
static patchpointer_t ttsoif[6][TTMAX_ALACROIX];
static patchpointer_t ttsoba[6][TTMAX_ALACROIX];
static patchpointer_t ttsobk[6][TTMAX_ALACROIX];
static patchpointer_t ttsodh[6][TTMAX_ALACROIX];
static patchpointer_t tttaib[6][TTMAX_ALACROIX];
static patchpointer_t tttaif[6][TTMAX_ALACROIX];
static patchpointer_t tttaba[6][TTMAX_ALACROIX];
static patchpointer_t tttabk[6][TTMAX_ALACROIX];
static patchpointer_t tttabt[6][TTMAX_ALACROIX];
static patchpointer_t tttaft[6][TTMAX_ALACROIX];
static patchpointer_t ttknib[6][TTMAX_ALACROIX];
static patchpointer_t ttknif[6][TTMAX_ALACROIX];
static patchpointer_t ttknba[6][TTMAX_ALACROIX];
static patchpointer_t ttknbk[6][TTMAX_ALACROIX];
static patchpointer_t ttkndh[6][TTMAX_ALACROIX];
...
// ttmode user
static patchpointer_t ttuser[TTMAX_USER];
static INT32 ttuser_count = 0;

static boolean goodending;
static patchpointer_t endbrdr[2]; // border - blue, white, pink - where have i seen those colours before?
static patchpointer_t endbgsp[3]; // nebula, sun, planet
static patchpointer_t endegrk[2]; // eggrock - replaced midway through good ending
static patchpointer_t endfwrk[3]; // firework - replaced with skin when good ending
static patchpointer_t endspkl[3]; // sparkle
static patchpointer_t endglow[2]; // glow aura - replaced with black rock's midway through good ending
static patchpointer_t endxpld[4]; // mini explosion
static patchpointer_t endescp[5]; // escape pod + flame
...
{
	endbrdr[1] = (patchpointer_t)W_GetPatchPointerFromName("ENDBRDR1", PU_PATCH);

	endegrk[0] = (patchpointer_t)W_GetPatchPointerFromName("ENDEGRK0", PU_PATCH);
	endegrk[1] = (patchpointer_t)W_GetPatchPointerFromName("ENDEGRK1", PU_PATCH);

	endglow[0] = (patchpointer_t)W_GetPatchPointerFromName("ENDGLOW0", PU_PATCH);
	endglow[1] = (patchpointer_t)W_GetPatchPointerFromName("ENDGLOW1", PU_PATCH);

	endbgsp[0] = (patchpointer_t)W_GetPatchPointerFromName("ENDBGSP0", PU_PATCH);
	endbgsp[1] = (patchpointer_t)W_GetPatchPointerFromName("ENDBGSP1", PU_PATCH);
	endbgsp[2] = (patchpointer_t)W_GetPatchPointerFromName("ENDBGSP2", PU_PATCH);

	endspkl[0] = (patchpointer_t)W_GetPatchPointerFromName("ENDSPKL0", PU_PATCH);
	endspkl[1] = (patchpointer_t)W_GetPatchPointerFromName("ENDSPKL1", PU_PATCH);
	endspkl[2] = (patchpointer_t)W_GetPatchPointerFromName("ENDSPKL2", PU_PATCH);

	endxpld[0] = (patchpointer_t)W_GetPatchPointerFromName("ENDXPLD0", PU_PATCH);
	endxpld[1] = (patchpointer_t)W_GetPatchPointerFromName("ENDXPLD1", PU_PATCH);
	endxpld[2] = (patchpointer_t)W_GetPatchPointerFromName("ENDXPLD2", PU_PATCH);
	endxpld[3] = (patchpointer_t)W_GetPatchPointerFromName("ENDXPLD3", PU_PATCH);

	endescp[0] = (patchpointer_t)W_GetPatchPointerFromName("ENDESCP0", PU_PATCH);
	endescp[1] = (patchpointer_t)W_GetPatchPointerFromName("ENDESCP1", PU_PATCH);
	endescp[2] = (patchpointer_t)W_GetPatchPointerFromName("ENDESCP2", PU_PATCH);
	endescp[3] = (patchpointer_t)W_GetPatchPointerFromName("ENDESCP3", PU_PATCH);
	endescp[4] = (patchpointer_t)W_GetPatchPointerFromName("ENDESCP4", PU_PATCH);
	...
	if ((goodending = ALL7EMERALDS(emeralds)))
	{
		...
		if (skins[skinnum].sprites[SPR2_XTRA].numframes > (XTRA_ENDING+2))
		{
			...
			endfwrk[0] = (patchpointer_t)W_GetPatchPointer(sprframe->lumppat[0], PU_PATCH);
			sprframe = &sprdef->spriteframes[XTRA_ENDING+1];
			endfwrk[1] = (patchpointer_t)W_GetPatchPointer(sprframe->lumppat[0], PU_PATCH);
			sprframe = &sprdef->spriteframes[XTRA_ENDING+2];
			endfwrk[2] = (patchpointer_t)W_GetPatchPointer(sprframe->lumppat[0], PU_PATCH);
		}
		else // Show a star if your character doesn't have an ending firework display. (Basically the MISSINGs for this)
		{
			endfwrk[0] = (patchpointer_t)W_GetPatchPointerFromName("ENDFWRK3", PU_PATCH);
			endfwrk[1] = (patchpointer_t)W_GetPatchPointerFromName("ENDFWRK4", PU_PATCH);
			endfwrk[2] = (patchpointer_t)W_GetPatchPointerFromName("ENDFWRK5", PU_PATCH);
		}

		endbrdr[0] = (patchpointer_t)W_GetPatchPointerFromName("ENDBRDR2", PU_PATCH);
	}
	else
	{
		// eggman, skin nonspecific
		endfwrk[0] = (patchpointer_t)W_GetPatchPointerFromName("ENDFWRK0", PU_PATCH);
		endfwrk[1] = (patchpointer_t)W_GetPatchPointerFromName("ENDFWRK1", PU_PATCH);
		endfwrk[2] = (patchpointer_t)W_GetPatchPointerFromName("ENDFWRK2", PU_PATCH);

		endbrdr[0] = (patchpointer_t)W_GetPatchPointerFromName("ENDBRDR0", PU_LEVEL);
	}
}

static void F_CacheGoodEnding(void)
{
	endegrk[0] = (patchpointer_t)W_GetPatchPointerFromName("ENDEGRK2", PU_PATCH);
	endegrk[1] = (patchpointer_t)W_GetPatchPointerFromName("ENDEGRK3", PU_PATCH);

	endglow[0] = (patchpointer_t)W_GetPatchPointerFromName("ENDGLOW2", PU_PATCH);
	endglow[1] = (patchpointer_t)W_GetPatchPointerFromName("ENDGLOW3", PU_PATCH);

	endxpld[0] = (patchpointer_t)W_GetPatchPointerFromName("ENDEGRK4", PU_PATCH);
}
...
void F_EndingDrawer(void)
{
	...
	else if (finalecount <= 20)
	{
		...
		if (finalecount && finalecount < 20)
		{
			...
			{
				trans = -trans;
				V_DrawScaledPatch(BASEVIDWIDTH/2, BASEVIDHEIGHT/2, 0, Patch_Dereference(endbrdr[0]));
			}
			V_DrawScaledPatch(BASEVIDWIDTH/2, BASEVIDHEIGHT/2, trans<<V_ALPHASHIFT, Patch_Dereference(endbrdr[1]));
		}
		else if (finalecount == 20)
			V_DrawScaledPatch(BASEVIDWIDTH/2, BASEVIDHEIGHT/2, 0, Patch_Dereference(endbrdr[0]));
	}
	...
	{
		V_DrawFill(24, 24, BASEVIDWIDTH-48, BASEVIDHEIGHT-48, 0);
		V_DrawFixedPatch(x+i, y+j, FRACUNIT, 0, Patch_Dereference(endegrk[0]),
	...
	else if (goodending && parallaxticker == -1)
	{
		...
		V_DrawScaledPatch(BASEVIDWIDTH/2, BASEVIDHEIGHT/2, 0, Patch_Dereference(endbrdr[1]));
	}
	else
	{
		...
		{
			V_DrawFixedPatch(-(x/10), -(y/10), FRACUNIT, 0, Patch_Dereference(endbgsp[0]), NULL); // nebula
			V_DrawFixedPatch(-(x/5),  -(y/5),  FRACUNIT, 0, Patch_Dereference(endbgsp[1]), NULL); // sun
			V_DrawFixedPatch(     0,  -(y/2),  FRACUNIT, 0, Patch_Dereference(endbgsp[2]), NULL); // planet

			// player's escape pod
			V_DrawFixedPatch((200<<FRACBITS)+(finalecount<<(FRACBITS-2)),
				(100<<FRACBITS)+(finalecount<<(FRACBITS-2)),
				FRACUNIT, 0, Patch_Dereference(endescp[4]), NULL);
			if (parallaxticker > -19)
			{
				...
				V_DrawFixedPatch((200<<FRACBITS)+(finalecount<<(FRACBITS-2)),
					(100<<FRACBITS)+(finalecount<<(FRACBITS-2)),
					FRACUNIT, trans<<V_ALPHASHIFT, Patch_Dereference(endescp[(finalecount/2)&3]), NULL);
			}
			...
			if (goodending && parallaxticker > 0) // gunchedrock
			{
				...
				// center detritrus
				V_DrawFixedPatch(i-x, j-y, FRACUNIT, 0, Patch_Dereference(endegrk[0]), colormap);
				if (trans < 10)
					V_DrawFixedPatch(i-x, j-y, FRACUNIT, trans<<V_ALPHASHIFT, Patch_Dereference(endegrk[0]), NULL);

				 // ring detritrus
				V_DrawFixedPatch((30*(FRACUNIT-scale))+i-(2*x), (30*(FRACUNIT-scale))+j-(2*y) - ((7<<FRACBITS)/2), scale, 0, Patch_Dereference(endegrk[1]), colormap);
				if (trans < 10)
					V_DrawFixedPatch((30*(FRACUNIT-scale))+i-(2*x), (30*(FRACUNIT-scale))+j-(2*y), scale, trans<<V_ALPHASHIFT, Patch_Dereference(endegrk[1]), NULL);

				scale += ((parallaxticker-10)<<7);

				 // shard detritrus
				V_DrawFixedPatch((30*(FRACUNIT-scale))+i-(x/2), (30*(FRACUNIT-scale))+j-(y/2) - ((7<<FRACBITS)/2), scale, 0, Patch_Dereference(endxpld[0]), colormap);
				if (trans < 10)
					V_DrawFixedPatch((30*(FRACUNIT-scale))+i-(x/2), (30*(FRACUNIT-scale))+j-(y/2), scale, trans<<V_ALPHASHIFT, Patch_Dereference(endxpld[0]), NULL);
			}
		}
		else if (goodending)
		{
		...
		if (parallaxticker <= 70) // eggrock/blackrock
		{
			...
			if ((-parallaxticker/4) < 5)
			{
				...
				if (trans < 0)
					trans = 0;
				V_DrawFixedPatch(x, y, scale, trans<<V_ALPHASHIFT, Patch_Dereference(endglow[(finalecount & 1) ? 0 : 1]), NULL);
			}
			...
			else
			{
				...
				V_DrawFixedPatch(x, y, scale, 0, Patch_Dereference(endegrk[0]), colormap[1]);
				...
				if ((-parallaxticker/2) < 5)
				{
					...
					if (trans < 0)
						trans = -trans;
					if (trans < 10)
						V_DrawFixedPatch(x, y, scale, trans<<V_ALPHASHIFT, Patch_Dereference(endegrk[1]), NULL);
				}
			}
		}
		else // firework
		{
			...
			if ((frame = ((parallaxticker & 1) ? 1 : 0) + (parallaxticker/TICRATE)) < 3)
				V_DrawFixedPatch(x, y, scale, 0, Patch_Dereference(endfwrk[frame]), colormap);
		}
		...
		if (sparklloop >= 3 && doexplosions)
		{
			...
			V_DrawFixedPatch(x + sparkloffs[0][0], y + sparkloffs[0][1],
				FRACUNIT, 0, Patch_Dereference(endxpld[sparklloop/4]), NULL);
		}
		...
		// border - only emeralds can exist outside it
		{
			...
			if (trans < 10)
				V_DrawScaledPatch(BASEVIDWIDTH/2, BASEVIDHEIGHT/2, trans<<V_ALPHASHIFT, Patch_Dereference(endbrdr[0]));
			if (borderstuff && parallaxticker < 11)
				V_DrawScaledPatch(BASEVIDWIDTH/2, BASEVIDHEIGHT/2, (parallaxticker-1)<<V_ALPHASHIFT, Patch_Dereference(endbrdr[1]));
			else if (goodending && finalecount > INFLECTIONPOINT && finalecount < INFLECTIONPOINT+10)
				V_DrawScaledPatch(BASEVIDWIDTH/2, BASEVIDHEIGHT/2, (finalecount-INFLECTIONPOINT)<<V_ALPHASHIFT, Patch_Dereference(endbrdr[1]));
		}
		...
		if (goodending && finalecount >= TICRATE && finalecount < INFLECTIONPOINT)
		{
			...
			for (i = 0; i < 7; ++i)
			{
				...
				while (j)
				{
					...
					//         1 -                   1 and 2
					//         2 -                   2 and not rendered
					V_DrawFixedPatch(x, y, FRACUNIT, 0, Patch_Dereference(endspkl[(j - ((sparklloop & 1) ? 0 : 1))]), colormap);
...
{ \
	arr[0] = (patchpointer_t)W_GetPatchPointerFromName(name, PU_LEVEL); \
	arr[min(1, maxf-1)] = 0; \
} \
...
{ \
	...
	{ \
		...
		if (lumpnum != LUMPERROR) \
			arr[i] = (patchpointer_t)W_GetPatchPointerFromName(lumpname, PU_LEVEL); \
		else \
			break; \
	} \
	arr[min(i, maxf-1)] = 0; \
...
static void F_CacheTitleScreen(void)
{
	switch(curttmode)
	{
		case TTMODE_OLD:
		case TTMODE_NONE:
			ttbanner = (patchpointer_t)W_GetPatchPointerFromName("TTBANNER", PU_LEVEL);
			ttwing = (patchpointer_t)W_GetPatchPointerFromName("TTWING", PU_LEVEL);
			ttsonic = (patchpointer_t)W_GetPatchPointerFromName("TTSONIC", PU_LEVEL);
			ttswave1 = (patchpointer_t)W_GetPatchPointerFromName("TTSWAVE1", PU_LEVEL);
			ttswave2 = (patchpointer_t)W_GetPatchPointerFromName("TTSWAVE2", PU_LEVEL);
			ttswip1 = (patchpointer_t)W_GetPatchPointerFromName("TTSWIP1", PU_LEVEL);
			ttsprep1 = (patchpointer_t)W_GetPatchPointerFromName("TTSPREP1", PU_LEVEL);
			ttsprep2 = (patchpointer_t)W_GetPatchPointerFromName("TTSPREP2", PU_LEVEL);
			ttspop1 = (patchpointer_t)W_GetPatchPointerFromName("TTSPOP1", PU_LEVEL);
			ttspop2 = (patchpointer_t)W_GetPatchPointerFromName("TTSPOP2", PU_LEVEL);
			ttspop3 = (patchpointer_t)W_GetPatchPointerFromName("TTSPOP3", PU_LEVEL);
			ttspop4 = (patchpointer_t)W_GetPatchPointerFromName("TTSPOP4", PU_LEVEL);
			ttspop5 = (patchpointer_t)W_GetPatchPointerFromName("TTSPOP5", PU_LEVEL);
			ttspop6 = (patchpointer_t)W_GetPatchPointerFromName("TTSPOP6", PU_LEVEL);
			ttspop7 = (patchpointer_t)W_GetPatchPointerFromName("TTSPOP7", PU_LEVEL);
			break;
...
static void F_UnloadAlacroixGraphics(SINT8 oldttscale)
{
	...
	{
		if (Patch_CanDereference(ttembl[oldttscale][i])) { Patch_FreeDereference(ttembl[oldttscale][i]); ttembl[oldttscale][i] = 0; }
		if (Patch_CanDereference(ttribb[oldttscale][i])) { Patch_FreeDereference(ttribb[oldttscale][i]); ttribb[oldttscale][i] = 0; }
		if (Patch_CanDereference(ttsont[oldttscale][i])) { Patch_FreeDereference(ttsont[oldttscale][i]); ttsont[oldttscale][i] = 0; }
		if (Patch_CanDereference(ttrobo[oldttscale][i])) { Patch_FreeDereference(ttrobo[oldttscale][i]); ttrobo[oldttscale][i] = 0; }
		if (Patch_CanDereference(tttwot[oldttscale][i])) { Patch_FreeDereference(tttwot[oldttscale][i]); tttwot[oldttscale][i] = 0; }
		if (Patch_CanDereference(ttrbtx[oldttscale][i])) { Patch_FreeDereference(ttrbtx[oldttscale][i]); ttrbtx[oldttscale][i] = 0; }
		if (Patch_CanDereference(ttsoib[oldttscale][i])) { Patch_FreeDereference(ttsoib[oldttscale][i]); ttsoib[oldttscale][i] = 0; }
		if (Patch_CanDereference(ttsoif[oldttscale][i])) { Patch_FreeDereference(ttsoif[oldttscale][i]); ttsoif[oldttscale][i] = 0; }
		if (Patch_CanDereference(ttsoba[oldttscale][i])) { Patch_FreeDereference(ttsoba[oldttscale][i]); ttsoba[oldttscale][i] = 0; }
		if (Patch_CanDereference(ttsobk[oldttscale][i])) { Patch_FreeDereference(ttsobk[oldttscale][i]); ttsobk[oldttscale][i] = 0; }
		if (Patch_CanDereference(ttsodh[oldttscale][i])) { Patch_FreeDereference(ttsodh[oldttscale][i]); ttsodh[oldttscale][i] = 0; }
		if (Patch_CanDereference(tttaib[oldttscale][i])) { Patch_FreeDereference(tttaib[oldttscale][i]); tttaib[oldttscale][i] = 0; }
		if (Patch_CanDereference(tttaif[oldttscale][i])) { Patch_FreeDereference(tttaif[oldttscale][i]); tttaif[oldttscale][i] = 0; }
		if (Patch_CanDereference(tttaba[oldttscale][i])) { Patch_FreeDereference(tttaba[oldttscale][i]); tttaba[oldttscale][i] = 0; }
		if (Patch_CanDereference(tttabk[oldttscale][i])) { Patch_FreeDereference(tttabk[oldttscale][i]); tttabk[oldttscale][i] = 0; }
		if (Patch_CanDereference(tttabt[oldttscale][i])) { Patch_FreeDereference(tttabt[oldttscale][i]); tttabt[oldttscale][i] = 0; }
		if (Patch_CanDereference(tttaft[oldttscale][i])) { Patch_FreeDereference(tttaft[oldttscale][i]); tttaft[oldttscale][i] = 0; }
		if (Patch_CanDereference(ttknib[oldttscale][i])) { Patch_FreeDereference(ttknib[oldttscale][i]); ttknib[oldttscale][i] = 0; }
		if (Patch_CanDereference(ttknif[oldttscale][i])) { Patch_FreeDereference(ttknif[oldttscale][i]); ttknif[oldttscale][i] = 0; }
		if (Patch_CanDereference(ttknba[oldttscale][i])) { Patch_FreeDereference(ttknba[oldttscale][i]); ttknba[oldttscale][i] = 0; }
		if (Patch_CanDereference(ttknbk[oldttscale][i])) { Patch_FreeDereference(ttknbk[oldttscale][i]); ttknbk[oldttscale][i] = 0; }
		if (Patch_CanDereference(ttkndh[oldttscale][i])) { Patch_FreeDereference(ttkndh[oldttscale][i]); ttkndh[oldttscale][i] = 0; }
	}
...
void F_TitleScreenDrawer(void)
{
	...
	switch(curttmode)
	{
		case TTMODE_OLD:
		case TTMODE_NONE:
			V_DrawSciencePatch(30<<FRACBITS, 14<<FRACBITS, 0, Patch_Dereference(ttwing), sc);

			if (finalecount < 57)
			{
				if (finalecount == 35)
					V_DrawSciencePatch(115<<FRACBITS, 15<<FRACBITS, 0, Patch_Dereference(ttspop1), sc);
				else if (finalecount == 36)
					V_DrawSciencePatch(114<<FRACBITS, 15<<FRACBITS, 0, Patch_Dereference(ttspop2), sc);
				else if (finalecount == 37)
					V_DrawSciencePatch(113<<FRACBITS, 15<<FRACBITS, 0, Patch_Dereference(ttspop3), sc);
				else if (finalecount == 38)
					V_DrawSciencePatch(112<<FRACBITS, 15<<FRACBITS, 0, Patch_Dereference(ttspop4), sc);
				else if (finalecount == 39)
					V_DrawSciencePatch(111<<FRACBITS, 15<<FRACBITS, 0, Patch_Dereference(ttspop5), sc);
				else if (finalecount == 40)
					V_DrawSciencePatch(110<<FRACBITS, 15<<FRACBITS, 0, Patch_Dereference(ttspop6), sc);
				else if (finalecount >= 41 && finalecount <= 44)
					V_DrawSciencePatch(109<<FRACBITS, 15<<FRACBITS, 0, Patch_Dereference(ttspop7), sc);
				else if (finalecount >= 45 && finalecount <= 48)
					V_DrawSciencePatch(108<<FRACBITS, 12<<FRACBITS, 0, Patch_Dereference(ttsprep1), sc);
				else if (finalecount >= 49 && finalecount <= 52)
					V_DrawSciencePatch(107<<FRACBITS, 9<<FRACBITS, 0, Patch_Dereference(ttsprep2), sc);
				else if (finalecount >= 53 && finalecount <= 56)
					V_DrawSciencePatch(106<<FRACBITS, 6<<FRACBITS, 0, Patch_Dereference(ttswip1), sc);
				V_DrawSciencePatch(93<<FRACBITS, 106<<FRACBITS, 0, Patch_Dereference(ttsonic), sc);
			}
			else
			{
				V_DrawSciencePatch(93<<FRACBITS, 106<<FRACBITS, 0, Patch_Dereference(ttsonic), sc);
				if (finalecount/5 & 1)
					V_DrawSciencePatch(100<<FRACBITS, 3<<FRACBITS, 0, Patch_Dereference(ttswave1), sc);
				else
					V_DrawSciencePatch(100<<FRACBITS, 3<<FRACBITS, 0, Patch_Dereference(ttswave2), sc);
			}

			V_DrawSciencePatch(48<<FRACBITS, 142<<FRACBITS, 0, Patch_Dereference(ttbanner), sc);
			break;
...
#undef KNUXY
		...
		case TTMODE_USER:
			...
			V_DrawSciencePatch(curttx<<FRACBITS, curtty<<FRACBITS, 0, Patch_Dereference(ttuser[ttuser_count]), sc);
